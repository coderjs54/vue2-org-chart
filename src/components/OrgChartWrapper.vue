<template>
  <div class="org-chart__wrapper">
    <div class="chart-wrapper">
      <OrgChart :orgData="orgData" ref="orgChartRef" :layout="layout" @zoom="handleZoom" />
    </div>
    <div class="zoom-wrapper">当前缩放：{{ zoomPercent }}</div>
...
export default {
  ...
  methods: {
    ...
    handleClickAdd() {
      this.$refs.orgChartRef.zoomUp()
    },
    handleClickMinus() {
      this.$refs.orgChartRef.zoomDown()
    },
    handleShowFull() {
      this.$refs.orgChartRef.showFull()
    },
    handleDownload() {
      this.$refs.orgChartRef.download()
    },
    handleChange() {
      let i = layouts.indexOf(this.layout)
      this.layout = layouts[++i % 2]
    }
  }
}
</script>
